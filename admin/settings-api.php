...
class Settings_API {
	 ...
	 * @var array
	 */
	protected $settings_prefix = 'jt_revolution_for_elementor_';
	...
	public function get_field_description( $args ) {
		if ( ! empty( $args['desc'] ) ) {
			$desc = sprintf( '<p class="description rfe-description">%s</p>', $args['desc'] );
		} else {
			$desc = '';
		}
	...
	function get_field_note( $args ) {
		if ( ! empty( $args['note'] ) ) {
			$note = sprintf( '<p class="note rfe-note">%1$s</p>', $args['note'] );
		} else {
			$note = '';
		}
	...
	function render_navigation() {
		$html = '<h2 class="nav-tab-wrapper rfe-nav-tabs">';
		...
		$classes = 'nav-tab rfe-nav-tabs__tab';
	...
	function render_forms() {
		?>
		<div class="metabox-holder rfe-metabox-holder rfe-settings">
			<?php foreach ( $this->settings_sections as $form ) {
				...
				?>
				<div id="<?php echo $form['id']; ?>" class="rfe-settings__group" style="display: none;">
					<form method="post" action="options.php" class="rfe-settings__form">
						<?php
